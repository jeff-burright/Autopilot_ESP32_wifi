/* Jeff's Arduino Autopilot Version ESP32_3.2. 
...
Version Notes: PID/deadband settings added. This version will attempt further code slimming.
...
//Libraries for ESP-NOW (smartwatch UI addition)
#include <esp_now.h>
#include <esp_wifi.h>
...
//float Kxte[3] = {.2, 0, 0}; // {.2, 4, .0004} baseline; {.05, .5, .0005}last used;  0 is proportional, 1 is differential, 2 is integral error, see GPS_Steer() in PID
 ...
 float MagVar_default = 7;// 18.4 Seattle   User should keep up to date for loaction.  Pgm will use GPS value if available + east, - west
...
//ledcSetup for IBT-2 motor controller (PWMChannel, PWMFreq, PWMResolution); //sets frequency (20kHz) and resolution (8bit) for motor controller PWM
