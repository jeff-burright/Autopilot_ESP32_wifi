/* Jeff's Arduino Autopilot Version ESP32_Tiller1.0 
Simple compass-only marine autopilot that functions like a Raymarine ST2000 or similar.
Stable on ESP32. Software support for IBT-2 motor controller, IR remote receiver, red button to toggle steering on/off, LSM303 9DOF compass, and 16x2 LCD. Rudder feedback via potentiometer optional.
If using Wifi for user interface, pilot is operational with only ESP32, IMU compass, motor controller, and motor. 
WIFI control of the pilot accessible by connecting a device to the pilot's SSID and directing browser to 192.168.4.1. Over the air updates may be uploaded via 192.168.4.1/update

Version Notes: Adjusted motorspeed minimum to fit the linear actuator & steering speed on my 21' boat.

Issues: pilot settings finicky in web browser. 
Experimental aspects: 
wiper motor noise reduction via PWM frequency modulation seems to work. It's left in for the linear actuator version here.
...
//float Kxte[3] = {.2, 0, 0}; // {.2, 4, .0004} baseline; {.05, .5, .0005}last used;  0 is proportional, 1 is differential, 2 is integral error, see GPS_Steer() in PID
 ...
 float MagVar_default = 7;// 18.4 Seattle; 14 Newport; 7 for installation on Puffin to match onboard compass. User should keep up to date for location.  Pgm will use GPS value if available + east, - west
