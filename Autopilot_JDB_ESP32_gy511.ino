/* Jeff's Arduino Autopilot Version ESP32_GY511. 
Simple compass-only WIFI-controlled marine autopilot that functions like a Raymarine ST2000 or similar.
Stable on ESP32. Software support for IBT-2 motor controller, IR remote receiver, red button to toggle steering on/off, LSM303 9DOF compass on GY-511 board, and 16x2 LCD. Rudder feedback potentiometer code included but not tested.
Using WIFI control, pilot is operational with only ESP32, IMU compass, motor controller, and motor. 
...
Fix bearing rate to make based on compass.read instead of magnetic values. Simplify!
...
#include <IRremote.hpp>
 ...
 #include <WiFi.h> 
#include <AsyncTCP.h> //wifi control
#include <ESPAsyncWebServer.h>  //wifit control
...
Serial.println(IP);          // default is 192.168.4.1
 ...
 //     initWebServer();        //not sure if needed
  ...
  server.on("/", HTTP_GET, [](AsyncWebServerRequest *request){
    ...
    //server.on("/readheading", handleheadrequest);//To get update of ADC Value only
